...
import kr.co.connect.boostcamp.livewhere.model.HouseInfo
...
class MapSearchRVAdapter(private val itemList: List<Any>?) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder.itemViewType == Category.HOUSE.type) {
            ...
            val houseItem = itemList?.get(position) as HouseInfo
        ...
        } else if (holder.itemViewType == Category.EMPTY.type) {
    ...
    override fun getItemViewType(position: Int): Int {
        if (itemList?.size!! > 0) {
            ...
            } else if (itemList[0] is EmptyViewHolder) {
                return Category.EMPTY.type
            }
        }
